...
class AutomationEngine:
    ...
    def add_manual_retry(self, product: dict):
        ...
        if url in self._workers:
            self.on_log(f"⚠️  Already queued: {product['name'][:40]}")
            return
...
class PokemonBotGUI:
    def __init__(self, root: tk.Tk):
        self.root        = root
        self.root.title("🎴 Pokémon Card Bot")
    ...
    def _build_ui(self):
        ...
        top.pack(fill="x")
        tk.Label(top, text="🎴 Pokémon Card Bot",
        ...
        for text, cmd, bg, fg in [
            ...
            ("⚙️ Settings",       self._open_settings,   "#0f3460", "#a8dadc"),
        ...
        for color, desc in [
            ...
            ("#f87171", "PRIO IV — Booster Boxes (most expensive, added last)"),
        ]:
    ...
    def _on_session_restored(self, retailer: str):
        ...
        if not self.engine._thread or not self.engine._thread.is_alive():
            self.engine.start()
            self._log("▶️  Session restored — bot resumed.")

    def _update_session_badge(self, retailer: str, ok: bool):
        badge = self._session_badges.get(retailer)
        if badge:
            badge.config(
                text=f"✅ {retailer.capitalize()}" if ok
                     else f"❌ {retailer.capitalize()} — Expired",
    ...
    def _on_settings_changed(self, new_values: dict):
        if "CHECK_INTERVAL" in new_values:
            ...
            self._log(f"⚙️  Check interval → {new_values['CHECK_INTERVAL']}s")
        if "RETRY_INTERVAL" in new_values:
            import retry_worker
            retry_worker.RETRY_INTERVAL = int(new_values["RETRY_INTERVAL"])
            self._log(f"⚙️  Retry interval → {new_values['RETRY_INTERVAL']}s")
        if "SESSION_CHECK_INTERVAL" in new_values:
            ...
            self._log(
                f"⚙️  Session interval → "
                f"{new_values['SESSION_CHECK_INTERVAL']}s")
        ...
        if "DEFAULT_BUDGET_LOW" in new_values or \
           "DEFAULT_BUDGET_HIGH" in new_values:
            lo, hi = self.settings.as_budget_range()
            self._log(f"⚙️  Default budget → ${lo}–${hi}")
